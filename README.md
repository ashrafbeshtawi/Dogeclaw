...
├── frontend/               # dev-only CSS toolchain (Tailwind v4 + daisyUI v5)
│   ├── package.json        # build/watch scripts — never shipped in the image
│   └── app.css             # theme + component styles → agent/src/web/public/app.css
...
The three UI pages (`login`, chat, admin) are styled with **Tailwind CSS v4 + daisyUI v5**.
The generated stylesheet (`agent/src/web/public/app.css`) is **committed**, so the
Docker image needs no frontend build step and deployments work fully offline.

Rebuild it after changing any page markup or `frontend/app.css`:
...
The theme (dark + DogeClaw red) is defined as a daisyUI theme in `frontend/app.css`.
Class names generated by the inline page JS (badges, chat bubbles, session items,
modals) are styled there too — they're load-bearing for the Playwright suite, so
rename them only together with the tests.

For a full design reference — tokens, component inventory, interaction
patterns, and the contracts a redesign must not break — see [DESIGN.md](DESIGN.md).
