...
var events = [];
var mcpServers = [], searchEngines = [], searchProviders = [];
...
function initialTab() {
    ...
    var valid = ['models','agents','skills','channels','mcp','search','crons','events','settings'];
...
async function load() {
    ...
    mcpServers = r[8].servers || [];
    searchEngines = r[9].engines || [];
    searchProviders = r[9].providers || [];
    renderModels(); renderAgents(); renderSkills(); renderChannels(); renderMcp(); renderSearchEngines(); renderCrons(); renderSettings();
...
function openMcpModal(s) {
    if (typeof s === 'number') s = find(mcpServers, s);
    ...
    document.getElementById('mcpAgentsCheckboxes').innerHTML = agents.map(function(a) {
...
function editMcp(id){ openMcpModal(id); }
...
function renderSearchEngines() {
    document.getElementById('searchEnginesTable').innerHTML = searchEngines.map(function(e, i) {
        ...
        return '<tr><td>'
            ...
            +'<button '+(i === searchEngines.length - 1 ? 'disabled' : '')+' onclick="moveSearchEngine('+e.id+',1)">↓</button> '
            +(i + 1)+'</td>'
...
function openSearchEngineModal(e) {
    if (typeof e === 'number') e = find(searchEngines, e);
    ...
    document.getElementById('searchEngineProvider').innerHTML = searchProviders.map(function(p) {
        return '<option value="'+esc(p)+'">'+esc(p)+'</option>';
    }).join('');
    document.getElementById('searchEngineProvider').value = e ? e.provider : searchProviders[0];
...
function editSearchEngine(id){ openSearchEngineModal(id); }
...
async function moveSearchEngine(id, dir) {
    var ids = searchEngines.map(function(e){ return e.id; });
...
function renderSkills() {
    document.getElementById('skillsTable').innerHTML = skills.map(function(s) {
        ...
        var access = ids.length === 0 ? '<span class="badge badge-sm badge-on">public</span>'
            : ids.map(function(id) { var a = find(agents, id); return '<span class="badge badge-sm badge-tag">' + esc(a ? a.name : '?') + '</span>'; }).join(' ');
        return '<tr><td><strong>' + esc(s.name) + '</strong></td>'
...
// Channel CRUD
function updateWebhookHint(channel) {
    var h = document.getElementById('webhookHint');
    if (siteConfig.telegramMode === 'polling') { h.textContent = 'Using polling mode (dev).'; return; }
    if (siteConfig.telegramMode !== 'webhook' || !siteConfig.webhookUrl) { h.textContent = ''; return; }
    h.innerHTML = channel && channel.webhook_id
        ? 'Webhook: <code>' + esc(siteConfig.webhookUrl) + '/webhook/' + esc(channel.webhook_id) + '</code>'
...
// Cron CRUD
// View state for the crons table: sub-tab (all/active/inactive) and
...
function setCronView(v) { cronView = v; renderCrons(); }

function setCronSort(key) {
...
function cronMatchesFilter(j, q) {
    if (!q) return true;
...
function cronFailed(j) { return !!j.last_run_at && j.last_status !== 'ok'; }

function cronTargetKey(j) {
    if (j.chat_id) return 'tg:' + (j.channel_name || '') + '/' + j.chat_id;
    if (j.session_id) return 'web:' + j.session_id;
    return '';
}

function cronSortValue(j, key) {
...
function syncCronAgentOptions(sel) {
    var seen = {}, opts = [];
    crons.forEach(function(j) {
...
function renderCrons() {
    ...
    var activeCount = crons.filter(function(j){ return j.enabled; }).length;
    var counts = {
        all: crons.length,
        active: activeCount,
        inactive: crons.length - activeCount,
        failing: crons.filter(cronFailed).length,
    };
    ...
    var shown = crons.filter(function(j) {
